...
describe('Test custom paginator directive', () => {
  ...
  it('Should show the right range numbers', () => {
    ...
    expect(rangeElement.innerHTML.trim()).toBe(
  ...
  it('Should disable the previous button and button [1] must be highlighted and disabled when current page is 1', () => {
    ...
    expect(
      firstPageButton.classList.contains('custom-paginator-page-disabled')
    ).toBe(true); // Disabled styles must be present
  ...
  it('Should highlight the current page button and it must be disabled', () => {
    ...
    expect(
      pageButton.classList.contains('custom-paginator-page-disabled')
    ).toBe(true); // Current page must be the highlighted
  });
  ...
  it('Should skip 2 pages forward when second dotted skip page button is clicked.', () => {
    ...
    fixture.detectChanges();
    const dottedNativeButtons = getNativeElementsByText(
      fixture,
      'button',
      '•••'
    );
    ...
    expect(
      currentPageButton.classList.contains('custom-paginator-page-disabled')
    ).toBe(true);
  ...
  it('Should skip 2 pages backward when first dotted skip page button is clicked.', () => {
    ...
    fixture.detectChanges();
    const currentPageButton = getElementByText(
      paginatorElement,
      'button',
      '94'
    ); // Current page must be 94 ((95 + 1) - 2 )
    expect(currentPageButton).toBeDefined(); // Page button must be present
...
function getNativeElementsByText(
  ...
  const nativeElements: HTMLElement[] = Array.from(
    fixture.nativeElement.querySelectorAll(type)
  );
